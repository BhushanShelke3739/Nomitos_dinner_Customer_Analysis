CREATE DATABASE milans_dinner;

ALTER DATABASE milans_dinner MODIFY NAME = nomitos_dinner;
...
WITH first_purchase_after_membership AS (
    ...
    WHERE s.order_date >= mb.joined_date
    GROUP BY s.customer_id
)
...
SELECT s.customer_id, SUM(
    CASE 
        WHEN s.order_date BETWEEN mb.joined_date AND DATEADD(day, 7, mb.joined_date) THEN m.price*20
        WHEN m.product_name = 'sushi' THEN m.price*20 
        ELSE m.price*10 
    END) AS total_points
FROM dbo.sales s
...
CASE WHEN s.order_date >= mb.joined_date THEN 'Y'
...
WITH customers_data AS (
	SELECT s.customer_id, s.order_date, m.product_name, m.price,
	CASE
		WHEN s.order_date < mb.joined_date THEN 'N'
		WHEN s.order_date >= mb.joined_date THEN 'Y'
		ELSE 'N' END AS member
	FROM sales s
	LEFT JOIN members mb ON s.customer_id = mb.customer_id
	JOIN menu m ON s.product_id = m.product_id
